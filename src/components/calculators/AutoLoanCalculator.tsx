...
export const DEFAULT_INPUTS: AutoLoanInputs = {
  vehiclePrice: 35000,
  tradeInValue: 12000,
  tradeInOwed: 0,
  downPayment: 3000,
...
const defaultFinanced = DEFAULT_INPUTS.vehiclePrice
  + (DEFAULT_INPUTS.vehiclePrice * DEFAULT_INPUTS.salesTaxRate / 100)
  - (DEFAULT_INPUTS.tradeInValue - DEFAULT_INPUTS.tradeInOwed)
...
export default function AutoLoanCalculator() {
  const [inputs, setInputs] = useState<AutoLoanInputs>(() => {
    ...
    if (!price && !tradein && !owed && !down && !tax && !rate && !term && !extra && !extrastart) return DEFAULT_INPUTS;
    return {
      ...
      tradeInOwed: owed ? parseFloat(owed) : DEFAULT_INPUTS.tradeInOwed,
  ...
  const netTradeEquity = useMemo(
    () => inputs.tradeInValue - inputs.tradeInOwed,
    [inputs.tradeInValue, inputs.tradeInOwed],
  );
  ...
  const financedAmount = useMemo(
    () => Math.max(0,
      inputs.vehiclePrice
      + (inputs.vehiclePrice * inputs.salesTaxRate / 100)
      - netTradeEquity
      - inputs.downPayment
    ),
    [inputs.vehiclePrice, inputs.salesTaxRate, netTradeEquity, inputs.downPayment],
  );

  const salesTaxAmount = useMemo(
    () => Math.round(inputs.vehiclePrice * inputs.salesTaxRate / 100 * 100) / 100,
    [inputs.vehiclePrice, inputs.salesTaxRate],
  );
  ...
  useEffect(() => {
    ...
    params.set("owed", inputs.tradeInOwed.toString());
  ...
  const handleInputChange = useCallback(
    (field: keyof AutoLoanInputs, value: string) => {
  ...
  const maxTradeIn = inputs.vehiclePrice;
  const maxDown = Math.max(0, inputs.vehiclePrice - netTradeEquity);
  const hasNegativeEquity = netTradeEquity < 0;
        ...
        <Card className="print-break-inside">
          <CardHeader className="pb-4">
            ...
            <p className="text-sm text-muted-foreground">
              Estimate monthly payments for a car purchase, including trade-in
              equity, negative equity rollover, down payment, and sales tax.
            </p>
          </CardHeader>
          <CardContent>
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="no-print">
                    ...
                    <Input
                      ...
                      className="mt-1"
                      aria-label="Vehicle price input"
                    />
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent side="top" className="max-w-[240px] text-xs">
                          The dealer&apos;s offer for your current vehicle.
                          This is subtracted from the amount you finance,
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <Label htmlFor="trade-in-owed" className="text-sm font-medium">
                        Amount Owed on Trade-In
                      </Label>
                      <Tooltip>
                        <TooltipTrigger asChild>
                        ...
                        <TooltipContent side="top" className="max-w-[280px] text-xs">
                          The remaining loan balance on your current vehicle.
                          If this exceeds the trade-in value, the difference
                          (negative equity) is added to your new loan. Set to
                          0 if you own the trade-in outright.
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    <span className="text-sm font-semibold text-foreground">
                      {formatCurrency(inputs.tradeInOwed)}
                    </span>
                  </div>
                  <div className="no-print">
                    <Slider
                      ...
                      max={50000}
                      step={500}
                      value={[inputs.tradeInOwed]}
                      onValueChange={([v]) =>
                        setInputs((p) => ({ ...p, tradeInOwed: v }))
                      }
                      aria-label="Amount owed on trade-in"
                      className="w-full"
                    />
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>{formatCurrency(0)}</span>
                    ...
                    <Input
                      ...
                      value={inputs.tradeInOwed}
                      onChange={(e) =>
                        handleInputChange("tradeInOwed", e.target.value)
                      }
                      className="mt-1"
                      aria-label="Amount owed on trade-in input"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="no-print">
                    ...
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>{formatCurrency(0)}</span>
                      <span>{formatCurrency(Math.max(0, maxDown))}</span>
                    </div>
                    <Input
                      type="number"
                      min={0}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent side="top" className="max-w-[280px] text-xs">
                          Rates vary by state, county, and city. Some states
                          tax the full purchase price; others exempt the
                          trade-in value. Enter the effective rate that
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5">
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent side="top" className="max-w-[240px] text-xs">
                                An additional amount paid each month toward
                                principal. This shortens the loan and reduces
                                total interest.
                              </TooltipContent>
                            </Tooltip>
                          </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <ResultCard
                        ...
                        subtext={`Price + tax ${hasNegativeEquity ? "+ rolled-over debt" : "- trade-in"} - down`}
                      />
                      {hasNegativeEquity && (
                        <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3">
                          <p className="text-xs font-medium text-destructive">Negative Equity Rollover</p>
                          <p className="mt-0.5 text-lg font-bold tracking-tight text-destructive">
                            {formatCurrency(Math.abs(netTradeEquity))}
                          </p>
                          <p className="text-[11px] text-muted-foreground">
                            Old debt added to new loan
                          </p>
                        </div>
                      )}
                      {!hasNegativeEquity && (
                        <ResultCard
                          ...
                          value={formatCurrency(netTradeEquity)}
                          subtext="Applied toward new purchase"
                        />
                      )}
                      <ResultCard
