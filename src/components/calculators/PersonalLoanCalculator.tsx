...
import {
  calculateLoan, calculateLoanWithExtra, calculateEffectiveAPR,
...
export const DEFAULT_INPUTS: PersonalLoanInputs = {
  loanAmount: 20000,
  apr: 12,
  termMonths: 60,
  originationFeePct: 0,
...
export default function PersonalLoanCalculator() {
  const [inputs, setInputs] = useState<PersonalLoanInputs>(() => {
    ...
    if (!amount && !rate && !term && !fee && !extra && !extrastart) return DEFAULT_INPUTS;
    return {
      loanAmount: amount ? parseFloat(amount) : DEFAULT_INPUTS.loanAmount,
      apr: rate ? parseFloat(rate) : DEFAULT_INPUTS.apr,
      termMonths: term ? parseInt(term, 10) : DEFAULT_INPUTS.termMonths,
      originationFeePct: fee ? parseFloat(fee) : DEFAULT_INPUTS.originationFeePct,
  ...
  const hasFee = inputs.originationFeePct > 0;
  const netProceeds = useMemo(
    () => inputs.loanAmount * (1 - inputs.originationFeePct / 100),
    [inputs.loanAmount, inputs.originationFeePct],
  );
  const originationFeeAmount = useMemo(
    () => inputs.loanAmount * inputs.originationFeePct / 100,
    [inputs.loanAmount, inputs.originationFeePct],
  );
  ...
  const effectiveAPR = useMemo(
    () => hasFee
      ? calculateEffectiveAPR(netProceeds, baseResult.monthlyPayment, inputs.termMonths)
      : inputs.apr,
    [hasFee, netProceeds, baseResult.monthlyPayment, inputs.termMonths, inputs.apr],
  );
  ...
  const handleInputChange = useCallback(
    (field: keyof PersonalLoanInputs, value: string) => {
        ...
        <Card className="print-break-inside">
          <CardHeader className="pb-4">
            ...
            <p className="text-sm text-muted-foreground">
              Estimate monthly payments, total interest, and effective APR
              for a personal loan. Optionally factor in an origination fee
              and extra monthly payments.
            </p>
          </CardHeader>
          <CardContent>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <Label htmlFor="origination-fee" className="text-sm font-medium">
                        Origination Fee
                      </Label>
                      <Tooltip>
                        ...
                        <TooltipContent side="top" className="max-w-[280px] text-xs">
                          Some lenders deduct a percentage of the loan amount
                          as an origination fee. You repay the full loan amount
                          but receive only the net proceeds after the fee.
                          Set to 0% if your loan has no origination fee.
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    <span className="text-sm font-semibold text-foreground">
                      {formatPercent(inputs.originationFeePct)}
                    </span>
                  </div>
                  <div className="no-print">
                    <Slider
                    ...
                    <Input
                      ...
                      value={inputs.originationFeePct}
                      onChange={(e) =>
                        handleInputChange("originationFeePct", e.target.value)
                      }
                      className="mt-1"
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent side="top" className="max-w-[240px] text-xs">
                                ...
                                payments. Set to 1 to start immediately, or
                                enter a higher number to begin after that
                                many regular payments.
                              </TooltipContent>
                            </Tooltip>
                          </div>
                          <span className="text-sm font-semibold text-foreground">
                ...
                {baseResult.schedule.length > 0 ? (
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {hasFee && (
                        <ResultCard
                          ...
                          subtext={`Loan ${formatCurrency(inputs.loanAmount)} - fee ${formatCurrency(originationFeeAmount)}`}
                        />
                      )}
                      {hasFee && (
                        <ResultCard
                          label="Effective APR"
                          value={formatPercent(effectiveAPR)}
                          subtext="Based on net proceeds"
                        />
                      )}
                      <ResultCard
                    ...
                    <div className="flex flex-wrap items-center gap-3 no-print">
                      ...
                      <ShareButtons summaryText={`${formatCurrency(inputs.loanAmount)} at ${formatPercent(inputs.apr)} over ${inputs.termMonths} months = ${formatCurrency(baseResult.monthlyPayment)}/mo. Calculate yours:`} />
                    </div>
